...
document.addEventListener("DOMContentLoaded",async()=>{
 const C=[44.45,12.02], STEP=15*60*1000, FUTURE=12;
 ...
 $("saveKey").onclick=()=>{let v=$("apiKey").value.trim();v?localStorage.setItem("mcTomorrowKey",v):localStorage.removeItem("mcTomorrowKey");keyUI();if(frames[idx]?.kind==="forecast")show(idx)};
 function tomorrowURL(t){return `https://api.tomorrow.io/v4/map/tile/{z}/{x}/{y}/precipitationIntensity/${encodeURIComponent(iso(t))}.png?apikey=${encodeURIComponent(key())}`}
 function replace(url,opacity=.72){return new Promise(resolve=>{let n=L.tileLayer(url,{maxZoom:12,opacity:0,crossOrigin:true,keepBuffer:1}).addTo(map),done=false;const finish=ok=>{if(done)return;done=true;if(ok){let old=layer;layer=n;n.setOpacity(opacity);if(old)setTimeout(()=>{try{map.removeLayer(old)}catch{}},120)}else{try{map.removeLayer(n)}catch{}}resolve(ok)};n.once("load",()=>finish(true));n.once("tileerror",()=>setTimeout(()=>finish(false),250));setTimeout(()=>finish(false),6500)})}
 async function show(n){idx=Math.max(0,Math.min(frames.length-1,n));$("timeline").value=idx;let f=frames[idx],future=f.kind==="forecast";$("badge").textContent=future?"PREVISIONE":"OSSERVATO";$("badge").className="badge"+(future?" future":"");$("clock").textContent=fmt(f.t);$("source").textContent=future?"Tomorrow.io · precipitazione":"RainViewer · radar";
  if(future){if(!key()){status("warn","Serve la chiave Tomorrow.io","Il radar passato è reale. Per visualizzare i frame futuri reali inserisci la API key Tomorrow.io nel campo sopra.");return}status("","Caricamento previsione",`Frame reale previsto ${fmt(f.t)}…`);let ok=await replace(tomorrowURL(f.t));status(ok?"ok":"warn",ok?"Previsione reale caricata":"Frame futuro non caricato",ok?`Precipitazione prevista alle ${fmt(f.t)}. Non è radar osservato.`:"Controlla la chiave Tomorrow.io o i limiti del piano.");}
  else{status("","Caricamento radar",`Frame radar ${fmt(f.t)}…`);let ok=await replace(`${rvHost}${f.path}/256/{z}/{x}/{y}/2/1_1.png`);status(ok?"ok":"warn",ok?"Radar reale caricato":"Radar non disponibile",ok?`Osservazione radar delle ${fmt(f.t)}.`:"Il frame RainViewer non ha risposto.");}}
 function stop(){if(timer)clearInterval(timer);timer=null;$("play").textContent="▶ Play"} function start(){stop();$("play").textContent="⏸ Pausa";timer=setInterval(()=>show(idx>=frames.length-1?0:idx+1),900)}
